...
def visualize_correlations(df: pd.DataFrame, voi: str) -> None:
    ...
    numeric_columns = numeric_columns[
        numeric_columns.columns.difference(["station_code"])
    ]
    correlation = numeric_columns.corr()
    plt.figure(figsize=(12, 12))
    sns.heatmap(correlation, cmap="coolwarm", fmt=".2f", vmin=-1, vmax=1, annot=True)
    plt.title(
        "Correlation heatmap of numerical significant features", fontsize="xx-large"
    )
...
def visualize_with_hue(df: pd.DataFrame, voi: str, hue_column: str) -> None:
    ...
    if hue_column == "precip_type":
        ...
        sns.lineplot(
            data=df, x="date", y=col[0], ax=ax, errorbar=None, hue=hue_column, alpha=0.5
        )
        ax.set_ylim(0, 75)
        ax.set_title("Time series of Precipitation (mm) with Precipitation type as hue")
        ax.set_ylabel("Precipitation (mm)")
        ax.set_xlabel("Date")
